...
There are several sources of information about this code. This code is described
in a research paper, Wu, Sean L., et al. "Vector bionomics and vectorial
capacity as emergent properties of mosquito behaviors and ecology." PLoS
computational biology 16.4 (2020): e1007446. That paper details the model. The
code, itself, has comments about what different parts represent. In this same
repository, outside of the MBITES project directory, are examples of how to use
the code. Here, I would like to add a description of the architecture of the
code.
    ...
    -   `Mosquitoes$apply(tag="MBITES")` is `mbites_MBITES()` in
        ...
        -   `oneBout()` MBITES-Bout.R:69
            ...
            -   `attempt_B()` in MBITES-Bout.R
                ...
                -   `site$sample_feed()` is `sample_feed_Site()` in
                    Landscape-Site.R:213.

                -   `self$chooseHost()` in MBITES-HostEncounter.R:41 which
                    samples the `RiskQ`

                -   `self$humanEncounter()` is `mbites_humanEncounter()`in in
                    MBITES-HostEncounter.R:68.
                    ...
                    -   `self$probeHost()`\# PATHOGEN-XX.R (mosy -\> host
                        ...
                        -   `get_human()$pushProbe()` in PATHOGEN-Null.R
                        ...
                        -   `probeHost_SEI()` in PATHOGEN-Generic-SIS.R.
                            ...
                            -   `get_human()$add_pathogen()` method in
                    ...
                    -   `self$feedHost()` \# PATHOGEN-XX.R
                        ...
                        -   `bloodFeed_SEI()` in PATHOGEN-Generic-SIS.R. Maybe?
                            It's not linked in the source code.
                    ...
                    -   `self$BloodMeal()` \# MBITES-BloodMeal.R. This is
                        `mbites_BloodMeal()` which is added to the mosquito.
                ...
                -   `self$zooEncounter()` is `mbites_zooEncounter()` in
                    MBITES-HostEncounter.R:115
            ...
            -   `updateState()` in MBITES-Bout.R
                ...
                -   `self$checkForResources()` in MBITES-Bout.R
                    ...
                    -   `self$BloodFeedingSearchCheck()` in MBITES-Bout.R. Checks if the local site has blood hosts present. Sets this in private$search. !!! If they aren't present, triggers a search.
