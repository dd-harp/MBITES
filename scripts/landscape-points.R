...
xy_f_files <- list.files(path = here("data/pointsets/"),pattern = "[[:alpha:]]+\\.f[0-9]{3}\\.xyw$")
xy_l_files <- list.files(path = here("data/pointsets/"),pattern = "[[:alpha:]]+\\.l[0-9]{3}\\.xyw$")
...
for(i in 1:n){
  xy_points[[i]]$f <- as.matrix(read.csv(paste0(here("data/pointsets/"),xy_f_files[i]),header = TRUE,sep = ","))
  xy_points[[i]]$l <- as.matrix(read.csv(paste0(here("data/pointsets/"),xy_l_files[i]),header = TRUE,sep = ","))
}
...
for(i in 1:n){
  ...
  write.table(x = dist,
              file = paste0(here("data/pointsets/"),"dist_",i,".csv"),
  ...
  write.table(x = xy_sites[[i]]$movement,
              file = paste0(here("data/pointsets/"),"movement_",i,".csv"),
              sep = ",",row.names = F,col.names = F)
...
saveRDS(object = landscapes,file = paste0(here("data/landscape/"),"periDomesticLandscapes.rds"))
saveRDS(object = xy_sites,file = paste0(here("data/landscape/"),"periDomesticRaw.rds"))
...
saveRDS(object = humans,file = paste0(here("data/landscape/"),"periDomesticHumans.rds"))
...
saveRDS(object = mosquitoes,file = paste0(here("data/landscape/"),"periDomesticMosquitoes.rds"))
